...
function chatgpt_assistant_messages_page(): void
{
	...
	// Retrieve the total number of messages using $wpdb->prepare
    ...
    // Retrieve the messages from the database with sorting and pagination using $wpdb->prepare for dynamic values
    ...
    <div class="container-fluid table-responsive">
        ...
        <table class="wp-list-table table table-auto table-hover previous-messages">
            ...
            foreach ($messages as $message) {
                ...
                echo '<td>' . esc_html($row_number) . '</td>'; // Display row number
        ...
        $page_1 = str_replace('class="page-numbers','class="page-link', esc_html($pages));
        $page_2 = str_replace('<a class="page-link"','<li class="page-item"><a class="page-link"', esc_html($page_1));
        $page_3 = str_replace('<a class="next','</li><li class="page-item"><a class="page-link next', esc_html($page_2));
        $page_4 = str_replace('<a class="prev','<li class="page-item"><a class="page-link prev', esc_html($page_3));
        $page_5 = str_replace('«</a>','«</a></li>', esc_html($page_4));
        $page_6 = str_replace('<span aria-current','<li class="page-item active"><span aria-current', esc_html($page_5));

        echo str_replace('</span>', '</span></li>', esc_html($page_6));
