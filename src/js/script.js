...
async function sendMessageAPI(location = null, message) {
    ...
    // Get the nonce value
    let nonce = document.getElementById('chatgpt_assistant_ajax_nonce_field').value;
    ...
    formData.append('nonce', nonce);  // Include the nonce value
    ...
    try {
        ...
        if (responseData.success) {
            ...
            nonce = document.getElementById('chatgpt_assistant_settings_nonce_field').value;
            if (addToSettings) {
                const settingsFormData = new FormData();
                settingsFormData.append('action', 'chatgpt_assistant_setting_action_callback');
                settingsFormData.append('chatgpt_assistant_setting_value', targetInputArea.value);
                settingsFormData.append('chatgpt_assistant_setting_key', targetInputArea.id);
                settingsFormData.append('nonce', nonce);
...
function nextStep() {
    stepper.next()
    // Wait for the element with id 'postTitleTextInput' to be present
    waitForElement('#postTitleTextInput', function (postTitleTextInput) {
        // Set the value of 'chatpgt_assistant_post_title' based on 'postTitleTextInput'
        document.querySelector('#chatpgt_assistant_post_title').value = postTitleTextInput.value;

        // Wait for the TinyMCE editor to be initialized
        waitForElement('#chatgpt_assistant_unique_editor_ifr', function () {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleChildBlur(event) {
        ...
        let nonce = document.getElementById('chatgpt_assistant_settings_nonce_field').value;

        // Create a new FormData object to hold the data to be sent
        const formData = new FormData();
        formData.append('action', 'chatgpt_assistant_setting_action_callback');
        formData.append('chatgpt_assistant_setting_value', inputValue);
        formData.append('chatgpt_assistant_setting_key', inputElement.id);
...
async function deleteSettingsAjax(location) {
    ...
    formData.append('chatgpt_assistant_setting_key', location);
    formData.append('nonce', nonce);
...
function waitForElement(selector, callback) {
    ...
    const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            ...
            if (addedNodes.some(node => node.matches && node.matches(selector))) {
                // Stop observing once the element is found
                observer.disconnect();

                // Execute the callback with the targetNode
